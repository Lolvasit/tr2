...
async def _confirm_make_mail(message: Message, state: FSMContext):
    ...
    setting.start_msg_id= 0 if not(message.text is None) else message.message_id
    setting.start_msg=message.text if not (message.text is None )else message.caption
    setting.save()
    await StartMailingStates.menu.set()
...
async def send_start_msg(send_to, setting_num):
    ...
    try:
        ...
        semd_msg = await bot.send_message( send_to, msg, reply_markup=start_kb) if (setting.start_msg_id == 0) else await bot.copy_message(
                send_to,
                setting.start_from_user_id,
                setting.start_msg_id,
                caption=msg,
        ...
        if setting_num == 1:
            ...
            scheduler.add_job(send_start_msg, trigger=DateTrigger(date), args=(send_to, 2), id=f"mail_after_start_{send_to}_{semd_msg.message_id}")
        if delete_time:
            date = datetime.now() + timedelta(seconds=delete_time.second, minutes=delete_time.minute, hours=delete_time.hour)
            scheduler.add_job(delete_msg, trigger=DateTrigger(date), args=(send_to, semd_msg.message_id), id=f"delete_msg_{send_to}_{semd_msg.message_id}")
